...
func (r *NotificationPostgres) CreateNotification(ctx context.Context, notification *models.Notification) error {
	query := `
        INSERT INTO notifications (id, channel, publication_at, title, message, sent_to_worker) 
        VALUES ($1, $2, $3, $4, $5)`

	_, err := r.db.ExecWithRetry(ctx, r.strategy, query, notification.ID, notification.Channel, notification.PublicationAt, notification.Content.Title, notification.Content.Message, notification.Sent)
	if err != nil {
		return err
	}
...
func (r *NotificationPostgres) UpdateNotification(ctx context.Context, newData *models.Notification) error {
	query := `
        ...
        SET channel = $1, publication_at = $2, title = $3, message = $4, sent_to_worker = $5, updated_at = now()
        WHERE id = $6`

	res, err := r.db.ExecWithRetry(ctx, r.strategy, query, newData.Channel, newData.PublicationAt, newData.Content.Title, newData.Content.Message, newData.Sent, newData.ID)
	if err != nil {
		return err
	}
	...
	if rowsAffected == 0 {
		return errors2.ErrNotificationNotFound
	}
	return nil
}
...
func (r *NotificationPostgres) GetNotification(ctx context.Context, id types.UUID) (*models.Notification, error) {
	query := `SELECT channel, publication_at, title, message, sent_to_worker FROM notifications WHERE id = $1`
	...
	var sent bool
	if err = row.Scan(&channel, &publishedAt, &title, &message, &sent); err != nil {
		return nil, err
	}
...
func (r *NotificationPostgres) Fetch(ctx context.Context, maxPublicationAt types.DateTime) ([]*models.Notification, error) {
	query := `SELECT id, channel, publication_at, title, message, sent_to_worker FROM notifications WHERE publication_at <= $1 ORDER BY publication_at`

	rows, err := r.db.QueryWithRetry(ctx, r.strategy, query, maxPublicationAt)
	if err != nil {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&idString, &channel, &publishedAt, &title, &message, &sent); err != nil {
...
func (r *NotificationPostgres) MarkAsSent(ctx context.Context, ids []*types.UUID) error {
	query := `UPDATE notifications SET sent_to_worker = $1 where id IN $2`

	// $2 requires to convert: []types.UUID to []string to ('string1', 'string2', ....)
	//
	...
	idsAsStr := fmt.Sprintf("('%s')", strings.Join(idsStrings, "','"))

	// exec
	_, err := r.db.ExecWithRetry(ctx, r.strategy, query, idsAsStr)
	if err != nil {
