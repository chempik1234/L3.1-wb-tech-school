...
func (r *NotificationPostgres) CreateNotification(ctx context.Context, notification *models.Notification) error {
	query := `
        INSERT INTO delayed_notifier.delayed_notifier.notifications (id, channel, publication_at, title, message, sent_to_worker) 
        VALUES ($1, $2, $3, $4, $5, $6)`

	_, err := r.db.ExecWithRetry(ctx, r.strategy, query, notification.ID.String(), notification.Channel.String(), notification.PublicationAt.String(), notification.Content.Title.String(), notification.Content.Message.String(), notification.Sent)
	if err != nil {
		return err
	}
...
func (r *NotificationPostgres) UpdateNotification(ctx context.Context, newData *models.Notification) error {
	...
	res, err := r.db.ExecWithRetry(ctx, r.strategy, query, newData.Channel.String(), newData.PublicationAt.String(), newData.Content.Title.String(), newData.Content.Message.String(), newData.Sent, newData.ID.String())
	if err != nil {
		return err
	}
...
func (r *NotificationPostgres) GetNotification(ctx context.Context, id types.UUID) (*models.Notification, error) {
	query := `SELECT channel, publication_at, title, message, sent_to_worker FROM delayed_notifier.delayed_notifier.notifications WHERE id = $1`
...
func (r *NotificationPostgres) Fetch(ctx context.Context, maxPublicationAt types.DateTime) ([]*models.Notification, error) {
	query := `SELECT id, channel, publication_at, title, message, sent_to_worker FROM delayed_notifier.delayed_notifier.notifications WHERE publication_at <= $1 AND sent_to_worker = false ORDER BY publication_at`
